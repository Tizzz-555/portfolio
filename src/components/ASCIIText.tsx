...
class CanvAscii {
  ...
  meshBaseWidth: number = 0;
  meshBaseHeight: number = 0;
  ...
  setMesh() {
    ...
    const planeH = baseH;
    this.meshBaseWidth = planeW;
    this.meshBaseHeight = planeH;
  ...
  fitMeshToViewport() {
    if (!this.mesh || !this.meshBaseWidth) return;

    // Fit long names into the camera frustum so text isn't clipped.
    const distance = this.camera.position.z - this.mesh.position.z;
    const fovInRadians = (this.camera.fov * Math.PI) / 180;
    const frustumHeight = 2 * Math.tan(fovInRadians / 2) * Math.abs(distance);
    const frustumWidth = frustumHeight * this.camera.aspect;
    // Keep extra horizontal breathing room on wide screens.
    const maxWidth = frustumWidth * 0.82;

    const scale = Math.min(1, maxWidth / this.meshBaseWidth);
